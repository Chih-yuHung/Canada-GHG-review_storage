...
#Make graph
Figure6a <- ggplot(GHG_liquid, aes(x = Pub..year, y = GHG, color = gas)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = c("CH4" = "darkmagenta",
                               ...
                               "N2O" = "goldenrod",
                               "NH3" = "violetred1")) +
  labs(x = "Publication Year", y = "Study Count", color = "GHG type") +
  theme_classic()+
  ...
  scale_x_continuous(limits = c(1990, 2024), 
                     ...
                     expand = c(0, 0)) +
  annotate("text", x = 1992, y = 110, 
           label = "(a)",size = 5)
Figure6a

Figure6b <- ggplot(GHG_solid, aes(x = Pub..year, y = GHG, color = gas)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = c("CH4" = "darkmagenta",
                                "CO2" = "limegreen",
  ...
  scale_x_continuous(limits = c(1990, 2024), 
                     breaks = seq(1990, 2024 , by = 5),
                     expand = c(0, 0)) +
  annotate("text", x = 1992, y = 67, 
           label = "(b)",size = 5)
Figure6b


#Export figure
ggsave("output/Figure - GHG type by solid and liquid manure.png", 
       ggarrange(Figure6a, Figure6b,
                 ncol = 1, nrow = 2),
