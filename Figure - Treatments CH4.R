...
#Make graph
Treatment_liquid <- ggplot(Treat_data_liquid, aes(x = Pub..year, y = Number, fill = Treatment)) +
  geom_bar(stat = "identity") +
  labs(x = "Publication Year", y = "Study count") +
  scale_fill_manual(values = c("No treatment" = "lightcyan", "Acidification" = "turquoise1", 
  ...
  scale_x_continuous(limits = c(1990, 2024), 
                     ...
                     expand = c(0, 0)) +
  annotate("text", x = 1991, y = 17.5, 
           label = "(a)",size = 6)
Treatment_liquid

Treatment_solid <- ggplot(Treat_data_solid, aes(x = Pub..year, y = Number, fill = Treatment)) +
  geom_bar(stat = "identity") +
  labs(x = "Publication Year", y = "Study count") +
  scale_fill_manual(values = c("No treatment" = "lightcyan", "Acidification" = "turquoise1", 
                               "Additives" = "deepskyblue2" , "Agitation" = "violet",
...
#Export figure
ggsave("output/Figure - Treatments (solid or liquid) CH4.png",
       ggarrange(Treatment_liquid, Treatment_solid,
                 ncol = 1, nrow = 2),
